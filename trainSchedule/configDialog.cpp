...
void ConfigDialog::openMap() {
    if ( ! isMapLoaded) {
        ...
        mapWidget->load(QUrl::fromLocalFile(htmlPath));
        mapWidget->hide();
...
void ConfigDialog::initializeMap() {
    ...
    if (mapWidget->page()->webChannel() != nullptr) {
        ...
        qDebug() << "exitsing channel";
    } else {
        channel = new QWebChannel(mapWidget->page());
        qDebug() << "missing channel";
    }
    channel->registerObject(QString("QTLatLineEdit"), latLineEdit);
    ...
    channel->registerObject(QString("QTLngLineEdit"), lngLineEdit);
    mapWidget->page()->setWebChannel(channel);
    ...
    mapWidget->getMapPage()->runJavaScript("initializeStartPosition();");
